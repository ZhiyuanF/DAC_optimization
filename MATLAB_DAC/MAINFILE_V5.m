...
user_input1 = input('Enter "NY" for New York, "CA" for California, or "TX" for Texas to load emission data: ', 's');
...
% New York, California, or Texas User Input for Emission Data
...
elseif strcmpi(user_input1, 'CA')
    ...
    co2_data = transpose(myISO{2:end, 11});  % Extract CO2 emissions from the 11th column  

elseif strcmpi(user_input1, 'TX')
    
    % Load emissions data from CSV for Texas
    myISO = readtable('TX_full_filtered_emission.csv');
    co2_data = transpose(myISO{1:end, 11});  % Extract CO2 emissions from the 11th column
...
% the title)
input_mat_file = 'expanded_RAW_TX.mat';
